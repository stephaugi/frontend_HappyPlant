...
const WaterCalendar = ({ plants, plantsToWater, allMoistureLogs, allWaterLogs }) => {
  ...
  console.log(allWaterLogs);

  const water_history = selectedDate && (selectedDate in allWaterLogs) ? allWaterLogs[selectedDate].map((moistureLog, index) => {
    return (
      <PlantAgenda
  ...
  const upcomingWaterDates = plantsToWater ? plantsToWater.map((plant, index) => {
    return <PlantAgenda key={`plantAgenda${index}`} upcomingWater={true} plant={plant} water={false}/>
  }) : null;
  ...
  return (<>
      ...
      <View style={[{ alignItems: "center" }]}>
        <Text style={fontStyles.header}>{selectedDate && (selectedDate in allMoistureLogs) ? "Events" : "Upcoming Waterings"}</Text>
        {agendas}
      </View>
      </ScrollView>
      </View>
